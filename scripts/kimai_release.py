...
Commands: detect, prepare, verify-manifest <file>, release.
The app's config.yaml and working run.sh are intentionally NOT supplied by this
patch. Only the top-level version in the existing config is changed.
"""
...
APP = ROOT / "kimai"
REPO = "Estirao/home-assistant-kimai"
...
def check_files() -> str:
    for name in ("config.yaml", "Dockerfile", "run.sh"):
        if not (APP / name).is_file():
            raise ValueError(f"Missing kimai/{name}. Keep the original working app files.")
...
def detect() -> None:
    ...
    write_outputs({"version": target, "upstream_tag": latest["tag_name"], **plan})
...
def main() -> None:
    ...
    parser.add_argument("command", choices=("detect", "prepare", "verify-manifest", "release"))
