...
# Use the actual server version, not a guess, in the disposable app configuration.
DB_VERSION="$(sql 'SELECT VERSION();')"
python3 - "$WORK/data/options.json" "$DB_VERSION" <<'PY'
import json, pathlib, sys
path = pathlib.Path(sys.argv[1])
options = json.loads(path.read_text())
options["database_version"] = sys.argv[2].strip()
